 ...
 * puntos — quedan siempre, sin scroll: PJ y las demás son el detalle.
 *
      ...
      <table className="w-full min-w-[520px] text-sm" data-testid={testId}>
        ...
        <thead>
          <tr className="bg-slate-50 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-600">
            <th scope="col" className="py-2 pl-3 pr-1 text-left font-bold">#</th>
            <th scope="col" className="py-2 pr-2 text-left font-bold">Equipo</th>
            {COLUMNAS.map((c) => (
            ...
            <th scope="col" className="py-2 pl-2 pr-3 text-right font-bold">Pts</th>
          </tr>
        </thead>
        <tbody>
          {filas.map((fila, i) => {
            ...
            return (
              ...
              >
                <td className="py-2 pl-3 pr-1 text-left font-bold tabular-nums text-slate-600">
                  {i + 1}
                </td>
                <th scope="row" className="py-2 pr-2 text-left font-semibold text-slate-900">
                  <span className="block max-w-[10rem] truncate">{fila.name}</span>
                </th>
                {COLUMNAS.map((c) => (
                ...
                <td className="py-2 pl-2 pr-3 text-right font-bold tabular-nums text-slate-900">
                  {fila.points}
                </td>
              </tr>
            );
