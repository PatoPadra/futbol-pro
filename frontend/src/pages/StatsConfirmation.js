...
import { AlertTriangle, ArrowLeft, CheckCircle, Clock, ClipboardList, Loader2, ThumbsUp } from 'lucide-react';
                ...
                {finalStats.map((s) => (
                  <div key={s.player_id} className="bg-white rounded-lg p-3 border border-turf/10" data-testid={`confirmed-stat-${s.player_id}`}>
                    ...
                    <div className="flex gap-4 mt-1.5 text-xs text-slate-600">
                      {s.goals > 0 && <span>Goles: {s.goals}</span>}
                      {s.assists > 0 && <span>Asist: {s.assists}</span>}
                      {s.saves > 0 && <span>Atajadas: {s.saves}</span>}
                          ...
                          >
                            <div className="min-w-0 flex-1">
                              <div className="flex gap-3 text-xs text-slate-600 flex-wrap">
                                ...
                                <span>Asistencias: {p.assists || 0}</span>
                                <span>Atajadas: {p.saves || 0}</span>
                              </div>
