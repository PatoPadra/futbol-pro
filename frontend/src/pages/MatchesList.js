...
const STATUS_LABELS = {
  abierto: 'Abierto', cerrado: 'Cerrado', equipos_generados: 'Equipos Generados',
  equipos_confirmados: 'Confirmado', finalizado: 'Finalizado', completado: 'Completado',
};
...
export default function MatchesList() {
  ...
  const upcoming = matches.filter(m => ['abierto', 'cerrado', 'equipos_generados', 'equipos_confirmados'].includes(m.status));
  const past = matches.filter(m => ['finalizado', 'completado'].includes(m.status));
      ...
      <div className="animate-slide-up">
        ...
        <Tabs defaultValue="proximos">
          <TabsList className="w-full grid grid-cols-2 h-12 bg-slate-100 rounded-xl mb-6">
            <TabsTrigger value="proximos" className="rounded-lg font-semibold text-sm">Proximos ({upcoming.length})</TabsTrigger>
            <TabsTrigger value="pasados" className="rounded-lg font-semibold text-sm">Pasados ({past.length})</TabsTrigger>
          </TabsList>
